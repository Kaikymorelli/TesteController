...
class HospedeControllerTest {
	 ...
	 @Autowired
	    ...
	    void testCriarHospede() {
	        ...
	        Assertions.assertEquals("Julia Maria", response.getBody().getNome());
	    }
	    ...
	    void testListarTodosHospedes() {
	        ...
	        // Assert
	        Assertions.assertEquals(HttpStatus.OK, response.getStatusCode(), "A resposta deveria ser 200 OK.");
	        Assertions.assertNotNull(response.getBody(), "O corpo da resposta não deveria ser nulo.");
	        Assertions.assertEquals(2, response.getBody().length, "A quantidade de hóspedes retornada deveria ser 2.");
	    }
	    ...
	    void testAtualizarHospede() {
	        ...
	        Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
	        Assertions.assertNotNull(response.getBody());
	        Assertions.assertEquals("Julia Silva", response.getBody().getNome());
	        Assertions.assertEquals("julia.silva@gmail.com", response.getBody().getEmail());
	    }
	    ...
	    void testDeletarHospede() {
	        ...
	        Assertions.assertEquals(HttpStatus.NOT_FOUND, checkDeleted.getStatusCode(), 
